"""Router-specific recorded fixtures for the deterministic suite + the CLI's default path.

The modes' own fixtures (``mode_b.fixtures``, ``mode_c.fixtures``, ``mode_a.fixtures``) key
recorded results by exact question, so a *blended* question — one the router fans out to
several modes — needs its own recorded entries in each mode it hits. These reuse the proofs'
real data (the Peskas validation passage; the Q4 Kwale catch-per-trip resolution) keyed
under the blended question. Mode A reads the real committed graph, so its part of a blended
question needs no fixture (the enumeration path is model-free).

Also here: the synthetic below-floor passages that pin the **reranker-logit-floor arm**
deterministically — a Norway off-topic question whose top passage carries a *negative
cross-encoder logit* (``rerank_score < RERANK_FLOOR``). The mode_b proof fixture's Norway
case retrieves **nothing** (the empty arm); this one exercises the distinct *score* arm with
no model or network. The live counterpart (real reranker downloads + runs) is the mandatory
end-to-end check.
"""
...
# synthesis) and C ("average total catch per trip" → structured query). Mode A reads the
# real graph (no fixture needed); B and C get recorded entries below.
BLENDED = (
...
# Mode-B recorded retrieval + synthesis for the blended question (reuses the proof's real
# Peskas validation passage from the SoftwareX PDF, p.2).
ROUTER_PASSAGES = {
    BLENDED: [
        Passage(
...
# Lamu 71.98 → Kilifi 22.93) — so the suite exercises the contract's ``figures`` payload,
# not just scalar claims. Routes to C alone ("average" + "by county").
...
# Mode-C recorded resolutions for the harness questions (the blended one reuses the proof's
# Q4: average tot_catch_kg per trip in Kwale, at trip grain).
ROUTER_RESOLUTIONS = {
...
# (RERANK_FLOOR = 0.0). The cross-encoder logit is negative → "not relevant" → the gate must
# refuse on the SCORE arm (not the empty arm).
Q_OFFTOPIC = "What is the impact of salmon cage farming on fjord water quality in Norway?"

OFFTOPIC_BELOW_FLOOR = {
    Q_OFFTOPIC: [
        # a real-ish Timor-Leste nutrition passage the bi-encoder might surface, but the
        # cross-encoder scores as irrelevant to a Norway salmon question
        Passage(
